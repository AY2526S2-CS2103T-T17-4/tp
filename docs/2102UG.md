...
* Quickly find any candidate by name, note, tag, or rejection reason
...
The box at the bottom of the window is where you type. Press **Enter** to run whatever you typed. The result appears just above the box.
...
|---|---|---|
| Name | `n/` | Letters and common punctuation (hyphens, apostrophes, periods, commas, parentheses). Must start with a letter. No digits. Up to 100 characters. |
| Phone | `p/` | Digits, with optional `+` at the start and optional spaces, hyphens, or parentheses as separators. Must contain 3–15 digits. Examples: `91234567`, `+6591234567`, `+65-9123-4567`. |
| Email | `e/` | Standard email format (`name@example.com`). Up to 254 characters. |
| Address | `a/` | Any non-empty text. Up to 100 characters. |
| Priority | `pr/` | `yes` (high priority) or `no` (normal). Defaults to `no`. |
| Tag | `at/` / `dt/` | Letters, digits, and the symbols `. + - _ ( ) @ # ! ? '`. No spaces. Up to 30 characters. |
...
Attaches a free-form, timestamped note to a candidate. Perfect for interview feedback, quick thoughts, or follow-up reminders — Talently automatically stamps each note with the date and time so you never lose track of when something happened.
